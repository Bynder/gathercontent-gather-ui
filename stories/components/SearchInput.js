...
storiesOf('Components', module).add('SearchInput', () => {
  ...
  return (
    <div>
      ...
      >
        <div className='flex'>
          <SearchInput onChangeHandler={action('change')} id="search-input" label="Search" value={value} onClearHandler={action('clear')}/>
        </div>
      </StoryItem>
      <StoryItem
      ...
      >
        <SearchInput onChangeHandler={action('change')} initialValue="foo" value={value} onClearHandler={action('clear')}/>
      </StoryItem>
    </div>
  );
});
