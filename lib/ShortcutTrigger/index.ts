...
function ShortcutTrigger({
  ...
  const checkCoreKeyRequirements = (event: any) => doesKeyMeetRequirements(withCtrlKey, event.ctrlKey) &&
  ...
  const shortcutHasBeenExecuted = (event: any) => event.key === shortcutKey ? checkCoreKeyRequirements(event) : false;
