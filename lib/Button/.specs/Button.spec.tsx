...
describe("Button", () => {
  it("Can have different classnames depending on the type", () => {
    ...
    expect(screen.getByRole('button').classList.contains('button--primary')).toBeTruthy();

    rerender(
      <Button types={["collapsed"]}>Botão</Button>
    );

    expect(screen.getByRole('button').classList.contains('button--collapsed')).toBeTruthy();
  });
  ...
  it("Should have the disabled attribute when clicked if props.disableOnClick is true", () => {
    ...
    rerender(
      <Button disableOnClick>Botão</Button>
    );
