...
class MentionsForm extends Component {
  ...
  onSubmit = e => {
    ...
    this.props.onInputChange('', this.getMentionsUsernames());
  };
  ...
  updateInputValue = e => {
    ...
    this.setState({ inputValue: value, mentionCount }, () => {
      this.props.onInputChange(value, this.getMentionsUsernames());
    });
  };

  getMentionsUsernames = () => {
    const mentions = this.removeMentionMarkup();

    const usernames = mentions.split('@');
    const usernamesWithBlanksRemoved = usernames.filter(username => username);
    return usernamesWithBlanksRemoved.map(username => username.trim());
  };
