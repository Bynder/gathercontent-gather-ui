...
class BoundaryClickWatcher extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = { boundaryIsActive: false, descendantHistory: [] };
    ...
    this.recordDescendantHistoryObserver = new MutationObserver(
      this.recordDescendantHistory
    );
  }
  ...
  componentDidMount() {
    ...
    this.recordDescendantHistory();
    this.recordDescendantHistoryObserver.observe(this.container, {
      childList: true,
      subtree: true
    });
  }
  ...
  componentWillUnmount() {
    ...
    this.recordDescendantHistoryObserver.disconnect();
  }

  recordDescendantHistory = () => {
    const descendantHistory = [
      ...this.container.querySelectorAll('*'),
      ...this.state.descendantHistory
    ];
    const uniqueDescendantHistory = uniqWith(
      descendantHistory,
      (firstNode, secondNode) => firstNode.isEqualNode(secondNode)
    );

    this.setState({ descendantHistory: uniqueDescendantHistory });
  };
  ...
  handleOuterClick(event) {
    ...
    this.setState({ descendantHistory: [] });
  }
  ...
  handleClick(event) {
    ...
    if (this.container) {
      containerDoesNotContainTarget =
        this.state.descendantHistory.indexOf(event.target) === -1;
