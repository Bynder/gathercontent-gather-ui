...
describe("CollectionsTableRow", () => {
  it("Should add modifier classes when props are set", () => {
    ...
    expect(screen.getByRole('row').classList.contains('collections-table__row--selected')).toBeTruthy();
    expect(screen.getByRole('row').classList.contains('collections-table__row--disabled')).toBeTruthy();
  });
});
...
describe("CollectionsTableCellContent", () => {
  it("Should add an overflow modifier class when props.allowOverflow is set", () => {
    ...
    expect(tableCell.querySelector('.collections-table__cell-content')?.classList.contains('collections-table__cell-content--allow-overflow')).toBeTruthy();
  });
});