...
const NotificationInlineBase = ({
  ...
  return (
    <div
      role="alert"
      className={`notification text-base font-semi-bold py-half px-threeQuarter border-2 border-solid inline-flex text-neutral-20 bg-white rounded items-center icon-inherit-color link-inherit-color border-${borderColor} ${classes} ${className}`}
      {...rest}
    >
      <Icon
        name={iconName}
