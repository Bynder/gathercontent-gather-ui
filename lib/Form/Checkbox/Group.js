...
export default class Group extends Component {
  static propTypes = {
    ...
    onAddingChoice: PropTypes.func,
    onRemovingChoice: PropTypes.func,
  ...
  addChoice(choice) {
    this.setState(
      prevState => ({ selected: prevState.selected.concat(choice) }),
      () => this.props.onAddingChoice(choice)
    );
  }

  removeChoice(choice) {
    this.setState(
      prevState => ({ selected: prevState.selected.filter(c => c !== choice) }),
      () => this.props.onRemovingChoice(choice)
    );
  }
