...
describe('ItemInput', () => {
  ...
  describe('Creating items', () => {
    it('creates a item containing text after entering text and pressing space', () => {
    ...
    it('creates an item containing text after entering text and blurring the input', () => {
      const { getByPlaceholderText, getByText } = renderComponent();
    ...
    it('handles pasting items into the input', () => {
      const {
      ...
      const expectedItems = emails.slice(0, indexOfLastEmail);
      expectedItems.forEach(email => {
        const itemElement = getByText(email);
        expect(isNodeInPill(itemElement)).toBe(true);
      });
