...
class DropdownContent extends Component {
  getStyle = () => {
    const { autoTopPosition, bounds, showContent } = this.context[
      GATHER_UI_DROPDOWN
    ];
    if (this.content && autoTopPosition && showContent) {
      ...
      return { top: bounds.top - 20 };
    }
    if (autoTopPosition && !showContent) {
      return {
        left: -99999,
        top: 'auto'
      };
