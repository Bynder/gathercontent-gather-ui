  ...
  .add('Nested V2', () => (
    <WindowingNestedContainer>
      {({ data }) => (
        <WindowingProvider
          style={{ overflow: 'scroll', position: 'relative', height: '500px' }}
          itemsLength={data.allIds.length}
          itemHeight={50}
        >
          <InViewList items={data.allIds} itemHeight={50} index={0}>
            {({ itemsInView }) =>
              itemsInView.map((i, index) => (
                <InViewListItem
                  key={i}
                  index={index}
                  style={{
                    paddingLeft: `${data.byId[i].depth * 40}px`
                  }}
                >
                  {data.childIds.indexOf(i) > -1 ? (
                    <ItemRow bordered>
                      <ItemRow.Name>{data.byId[i].name} </ItemRow.Name>
                    </ItemRow>
                  ) : (
                    <FolderRow open>
                      {(show, setShow) => (
