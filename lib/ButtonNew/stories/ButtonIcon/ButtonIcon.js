import React from 'react';
import { storiesOf } from '@storybook/react';
import { ButtonIconDanger } from 'lib';
import { ButtonIcon } from '../../ButtonIcon/ButtonIcon';
import { ButtonIconContained } from '../../ButtonIcon/ButtonIconContained';
import StoryItem from '../../../../stories/styleguide/StoryItem';
import { ButtonStoryItem } from '../ButtonStoryItem';
import { Counter } from '../../../Counter';

storiesOf('Components', module).add('ButtonIcon', () => (
  <StoryItem title="ButtonIcon" description="The icon button component">
    <div className="flex flex-wrap justify-around">
      <ButtonStoryItem title="MD">
        <ButtonIcon name="blockquote24" size={ButtonIcon.sizes.md} />
      </ButtonStoryItem>

      <ButtonStoryItem title="SM">
        <ButtonIcon name="caption16" size={ButtonIcon.sizes.sm} />
      </ButtonStoryItem>

      <ButtonStoryItem title="Disabled">
        <ButtonIcon name="caption16" disabled />
      </ButtonStoryItem>

      <ButtonStoryItem title="Active">
        <ButtonIcon name="caption16" active />
      </ButtonStoryItem>

      <ButtonStoryItem title="Enabled">
        <ButtonIcon name="caption16" enabled />
      </ButtonStoryItem>

      <ButtonStoryItem title="Counter">
        <ButtonIcon name="caption16">
          <Counter>9</Counter>
        </ButtonIcon>
      </ButtonStoryItem>

      <ButtonStoryItem title="Danger">
        <ButtonIconDanger name="caption16" />
      </ButtonStoryItem>

      <ButtonStoryItem title="Contained">
        <ButtonIconContained name="caption16" />
      </ButtonStoryItem>
    </div>
  </StoryItem>
));
