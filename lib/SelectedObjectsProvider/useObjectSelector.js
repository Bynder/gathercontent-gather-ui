...
export function useObjectSelector(
  ...
  const handleMouseEnter = idsToSelect
    ? () => setIntendedToSelect(getIds(intendedToSelect))
    : () => setIntendedToSelect(getIds(intendedToSelect));

  const handleMouseLeave = idsToSelect
    ? () =>
        setIntendedToSelect(
          intendedToSelect.filter(i => idsToSelect.indexOf(i) !== -1)
        )
    : () => setIntendedToSelect(intendedToSelect.filter(i => i !== id));
