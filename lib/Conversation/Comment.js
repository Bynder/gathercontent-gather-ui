...
class Comment extends Component {
  ...
  highlightMentions(string) {
    const pattern = /(\B\@\w+)+/gi;
    let strArr = string.split(pattern);
    return strArr.map((subStr, i) => {
      if(subStr.match(pattern)){
        return <span key={`match${i}`} className="mention">{subStr}</span>
      } else {
        return subStr;
      }
    });
        ...
        <div className="conversation__comment-body">
          {!this.state.showEditForm && (
            <p className="conversation__text">
              <Linkify>{this.highlightMentions(body)}</Linkify>
            </p>
          )}
