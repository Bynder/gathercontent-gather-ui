...
function Thumb({ src, altText, title, fallback, children, ...props }) {
  ...
  const [
    minimumLoadingTimeSurpassed,
    setMinimumLoadingTimeSurpassed
  ] = useState(true);
  useEffect(() => {
    setTimeout(() => setMinimumLoadingTimeSurpassed(true), 250);
  }, []);
  const imageIsLoading =
    minimumLoadingTimeSurpassed && image.isLoading && !image.error;
  const imageHasSucceeded =
    minimumLoadingTimeSurpassed && !image.isLoading && !image.error;
  const imageHasFailed =
    minimumLoadingTimeSurpassed && image.error && !image.isLoading;
