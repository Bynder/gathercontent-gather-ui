...
import emptyCircleSVG from '../../assets/icons/empty.svg';
...
import quoteStartSVG from '../../assets/icons/quoteStart.svg';
import embedSVG from '../../assets/icons/embed.svg';
...
const Icon = ({
  ...
  if (!IconSVG) {
    console.warn(`The ${name} icon does not exist in this library`); // eslint-disable-line no-console
    return null;
  }
  ...
  useEffect(() => {
    ...
    /*
    const svgElement = iconElement.current.querySelector('svg')
  ...
  const classNames = cx(
    className,
    createClassesFromTypesList(types, 'icon--'),
    {
      'icon--hide-text': hideText,
      'icon--has-notification': notification
    }
  );

  const textClassNames = cx(
    'icon__text',
    createClassesFromTypesList(textTypes, 'icon__text--')
  );
  return (
    <span className={`icon icon--${name} ${classNames}`}>
      {notification && (
        <span className="icon__notification">{notification}</span>
      )}
      <span className="icon__body">
        <span className="icon__image" ref={iconElement}>
          <IconSVG />
        </span>
