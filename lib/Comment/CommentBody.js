...
function BlurBottom() {
  return (
    <span className="absolute h-8 w-full top-0 left-0 mt-24 bg-blur-white-bottom conversation__text__cutoff" />
  );
}
    ...
    <div className={`${isDeleting && 'blur-background'}`}>
      <div className="mb-2">
        <Person person={comment.author} className="conversation">
          {createdAt && (
            <div className="text-sm mr-1 font-roman text-neutral-primary">
              {createdAt}
            </div>
          )}
        </Person>
      ...
      <div
        className={`mb-0 mx-0 pr-0 py-0 text-neutral-20 break-words whitespace-pre-line relative ${
