...
The library will automatically deploy when a new tag is pushed to the master branch. You can create a tag using the web interface, using the command line with [git tags](https://git-scm.com/book/en/v2/Git-Basics-Tagging), or running `npm version  <major | minor | patch>` (We usually go with patch, but minor or major may be required) will also create a tag for you. Just ensure when pushing you use `git push --tags`.
